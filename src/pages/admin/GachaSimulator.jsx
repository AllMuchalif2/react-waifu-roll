...
import {
  ...
  getFlashClassByTier,
  getBestTier,
} from '../../lib/gachaEffects';

const MOCK_WAIFUS = {
  C: {
    name: 'Mock Common',
    tier: 'C',
    image_url: 'https://placehold.co/400x400/adb5bd/white?text=Common',
  },
  B: {
    name: 'Mock Uncommon',
    tier: 'B',
    image_url: 'https://placehold.co/400x400/51cf66/white?text=Uncommon',
  },
  A: {
    name: 'Mock Rare',
    tier: 'A',
    image_url: 'https://placehold.co/400x400/339af0/white?text=Rare',
  },
  R: {
    name: 'Mock Rare+',
    tier: 'R',
    image_url: 'https://placehold.co/400x400/cc5de8/white?text=Rare+',
  },
  S: {
    name: 'Mock Special',
    tier: 'S',
    image_url: 'https://placehold.co/400x400/f06595/white?text=Special',
  },
  SR: {
    name: 'Mock Super Rare',
    tier: 'SR',
    image_url: 'https://placehold.co/400x400/ff922b/white?text=SR',
  },
  SSR: {
    name: 'Mock SSR',
    tier: 'SSR',
    image_url: 'https://placehold.co/400x400/fcc419/1a1a1a?text=SSR',
  },
  UR: {
    name: 'Mock Ultra Rare',
    tier: 'UR',
    image_url: 'https://placehold.co/400x400/ff6b6b/white?text=UR',
  },
  LIMITED: {
    name: 'Mock Limited',
    tier: 'LIMITED',
    image_url: 'https://placehold.co/400x400/ff6b6b/white?text=Limited',
  },
};
    ...
    setTimeout(() => {
      ...
      if (count === 1) {
        ...
        for (let i = 0; i < 9; i++) {
          ...
          results.push({
            ...MOCK_WAIFUS[randTier],
            name: `Random ${randTier} #${i + 1}`,
          });
        }
    ...
    <>
      ...
      <div className="min-h-screen">
        ...
        <main className="px-6 max-w-lg mx-auto pb-32 pt-6">
          <div className="card-neo bg-primary text-white mb-8 py-4 shadow-[6px_6px_0px_#ffea00]">
          ...
          <div className="card-neo mb-6 p-4">
            <h3 className="text-[0.65rem] font-black uppercase mb-3 text-primary italic">
              Simulasi Per Tier (1X)
            </h3>
              ...
              {Object.keys(TIER_CONFIG).map((tier) => {
                ...
                return (
                  <button
                    ...
                    disabled={isFetching}
                    className={`p-2 border-2 border-border-main rounded-xl text-[0.6rem] font-black uppercase transition-all shadow-[3px_3px_0px_var(--border)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none ${config.color} ${config.textColor} ${isFetching ? 'opacity-50 grayscale' : 'hover:-translate-y-0.5'}`}
                  >
                    {tier}
                  </button>
                );
            ...
            <div className="mt-4 pt-4 border-t-2 border-border-main/10">
              <button
                ...
                disabled={isFetching}
                className="btn-neo btn-neo-secondary w-full py-3 text-xs text-primary"
              >
                <i className="fa-solid fa-layer-group mr-2"></i> SIMULASI 10X
                (Mix)
              </button>
            </div>
          </div>
          ...
          <div
            className={`relative min-h-[300px] flex flex-col items-center justify-center border-4 border-dashed border-border-main/20 rounded-3xl bg-card-bg/50 p-4`}
          >
            {isFetching ? (
              <div className="text-center">
                <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                <p className="text-sm font-black text-primary animate-pulse uppercase">
                  Mengacak...
                </p>
              </div>
            ) : result ? (
              <div
                className={`w-full ${isMultiple ? 'max-w-md' : 'max-w-[280px]'} animate-zoom-in flex flex-col items-center`}
              >
                <div className="card-neo bg-white p-4 w-full">
                  ...
                  <div className="mt-4 pt-3 border-t-2 border-border-main text-center">
                    <span className="text-[0.65rem] font-black uppercase italic text-text-muted">
                      Simulator Preview
                    </span>
                  </div>
                </div>
              ...
              <div className="text-center text-text-muted opacity-50 px-10">
                ...
                <p className="text-xs font-black uppercase italic">
                  Pilih tier di atas untuk mensimulasikan animasi gacha
                </p>
              </div>
            )}
          </div>
